...
// Bridge XRPL funds to the EVM sidechain via Axelar's General Message Passing.
//
// A "shield" is an Axelar *interchain transfer with payload*: an XRPL Payment to
// the Axelar gateway multisig, carrying memos that tell the Axelar network which
// chain/address to deliver to, plus a `payload`. Relayers pick it up off the
// ledger, mint the bridged token to `destination_address` (our AxelarPoolRouter)
// on `destination_chain`, and call its `_executeWithInterchainToken` with the
// payload — which decodes to a RAILGUN `ShieldRequest[]` and shields the funds.
//
// See: axelarnetwork/axelar-contract-deployments → xrpl/interchain-transfer.js
//      (memo layout) and lib/shield-payload.js (the payload).

// Axelar gateway multisig on the XRPL **mainnet**. The Payment Destination.
// Source: axelar-chains-config/info/mainnet.json
//         → chains.xrpl.contracts.AxelarGateway.address
export const AXELAR_GATEWAY = "rfmS3zqrQrka8wVyhXifEeyTwe8AMz2Yhw";

// The XRPL EVM sidechain, as named by Axelar (chains.xrpl-evm.axelarId).
export const DESTINATION_CHAIN = "xrpl-evm";

// Our AxelarPoolRouter on the EVM sidechain — the hard-coded recipient of every
// shield. The router receives the bridged token and forwards it into the RAILGUN
// shielded pool (pool.shield) in the same call.
